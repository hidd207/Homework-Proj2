...
# 題目敘述
(a)
有 $n$ 筆紀錄需要在一台記憶體容量為 $S$ 筆紀錄（$S \ll n$）的電腦上排序。假設整個 $S$ 筆的記憶體都可以用來作為輸入/輸出緩衝區。輸入資料存放在磁碟上，並且一共分成 $m$ 個區段（runs）。假設每次磁碟存取時，搜尋時間為 $t_s$，延遲時間為 $t_l$，每筆資料的傳輸時間為 $t_t$。如果在外部排序的第二階段採用 $k$ 路歸併，並且將內部記憶體分割成輸入/輸出緩衝區，讓輸入、輸出與 CPU 處理可以重疊執行（如 7.21 範例中的 Buffering），請問此階段的總輸入時間是多少？  
(b)
