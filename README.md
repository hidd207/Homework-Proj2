...
# 題目敘述
請寫一個 C++ 抽象類別（abstract class）針對 ADT MinPQ用來定義一個最小優先佇列。接著，寫一個繼承這個抽象類別的 C++ 類別 MinHeap，並實作 MinPQ 所有的虛擬函式。每個函式的時間複雜度應該和 MaxHeap 的對應函式相同。
...
本題針對外部排序第二階段的 k-way merge 進行理論推導與效能分析。  
外部排序是一種針對資料量遠大於主記憶體時的排序解決方案，分為產生初始runs及merge兩階段。本題重點在於分析merge的 I/O 輸入時間，並討論調整 merge 段數 k 對總 I/O 時間的影響。  
經由推導可知，總輸入時間 $t_{input}$ 隨 k 線性成長。這是因為每增加一個 merge 通道，就需更多次磁碟 seek 與 latency 操作，使 I/O 成本提高。而每筆資料的傳輸成本則與資料總量 n 成正比，與 k 無關。  
因此，t_input 的增長主要由 seek/latency 部分主導。
實際運作時，若 k 設得太小，merge 階段次數增加，導致總排序所需週期增多；反之，k 太大又會因 I/O 負擔加重而拖慢進度。理論上，應選取一個平衡點，使得 CPU 處理與 I/O 時間相當，達到資源利用最佳化。
透過程式驗證，發現 t_input = 10k + 200，k 每增加 1，輸入時間增加 10 秒，反映出 I/O 效能瓶頸的現象。這有助於預估不同系統下外部排序的運作效率，並指引實際系統設計時 k 值的合理選擇。  
本題練習了數學建模、效能推導及 C++ 程式設計，讓我更加理解大數據排序中的理論與實務平衡點，對資料結構與演算法分析能力有明顯提升。  
